...
import com.heima.exception.ResultReturnException;
import com.heima.service.impl.PlateServiceImpl;
...
public class FileUtil {
    private static final SnowflakeIdWorker idWork = new SnowflakeIdWorker(1, 1);
    private static String baseImageUrl = PlateServiceImpl.class.getClassLoader().getResource("images/").getPath().substring(1);
    ...
    private static String getImageUrlExtension(String url) {
    ...
    private static String getRandomFileName(String url) {
        return baseImageUrl + idWork.nextId() + "." + getImageUrlExtension(url);
    }

    //java 通过url下载图片保存到本地
    public static String downloadImage(String urlString) {
        // 构造URL
        String filename = getRandomFileName(urlString);  //下载路径及下载图片名称
        try {

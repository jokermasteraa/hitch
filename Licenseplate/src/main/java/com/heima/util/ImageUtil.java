...
public class ImageUtil {
     ...
     * @param dst
     */
    public static void canny(Mat inMat, Mat dst) {
        // 低于阈值1的像素点会被认为不是边缘；
        // 高于阈值2的像素点会被认为是边缘；
     ...
     * @return
     */
    public static void threshold(Mat inMat, Mat dst) {
    ...
    public static Mat morphologyClose(Mat inMat, Mat dst) {
     ...
     * @return
     */
    public static Mat morphologyOpen(Mat inMat, Mat dst) {
     ...
     * @return
     */
    public static List<MatOfPoint> contours(Mat src, Mat inMat) {
     ...
     * @return
     */
    public static Vector<Mat> screenBlock(Mat src, List<MatOfPoint> contours, Boolean isGreen){
    ...
    private static Size shearCorrection(MatOfPoint2f m2, RotatedRect mr, Mat inMat, Mat dst, Size rect_size){
        Mat vertex = new Mat();
        ...
        List<Point> result = new ArrayList<>(); // 按坐标有序
     ...
     */
    public static void shearCorrection(Mat inMat, Mat dst, Integer px){
        MatOfPoint2f srcPoints = new MatOfPoint2f();
        srcPoints.fromArray(new Point(0, 0), new Point(0, inMat.rows()), new Point(inMat.cols(), 0));
        MatOfPoint2f dstPoints = new MatOfPoint2f();
     ...
     * @return
     */
    public static Mat dilate(Mat inMat, int row, int col, Boolean correct) {
     ...
     * @return
     */
    public static Mat equalizeHist(Mat inMat) {
        Mat dst = new Mat();
        // 灰度图均衡化
     ...
     * @return
     */
    public static Mat equalizeHist1(Mat inMat, Mat dst) {
     ...
     */
    public static Mat hsvFilter(Mat inMat, Integer...range) {
     ...
     * @return 返回二值图像
     */
    public static Mat hsvThreshold(Mat inMat, Integer...range) {
        Mat hsvMat = new Mat();    // 转换为hsv图像
        Imgproc.cvtColor(inMat, hsvMat, Imgproc.COLOR_BGR2HSV);
     ...
     * @return
     */
    public static Mat narrow(Mat inMat, Integer maxCols) {
     ...
     * @return
     */
    public static void enlarge(Mat inMat, Mat dst, Size size) {
     ...
     * @return
     */
    public static void rotateImg(Mat inMat, Mat dst, double angle){
        Point src_center = new Point(inMat.cols() / 2.0F, inMat.rows() / 2.0F);
        rotateImg(inMat, dst, angle, src_center);
    }
     ...
     * @param dst
     */
    public static void edgeEnhance(Mat inMat, Mat dst) {
        List<Mat> matList = new ArrayList<>();
        Core.split(inMat, matList);
        for (Mat mat : matList) {
     ...
     */
    public static void unevenLightCompensate(Mat grey, Mat dst, Integer blockSize) {

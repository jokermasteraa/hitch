...
import com.heima.constant.Constant;
import com.heima.entity.PlateRecoResult;
import com.heima.enumtype.Direction;
import com.heima.enumtype.PlateColor;
import com.heima.enumtype.PlateHSV;
import com.heima.train.SVMTrain;
...
public class PlateUtil {
     ...
     */
    public static Vector<Mat> findPlateByContours(Mat src, Mat inMat, Vector<Mat> dst) {
        // 灰度图
        Mat gray = new Mat();
        ImageUtil.gray(inMat, gray);

        // 高斯模糊
        Mat gsMat = new Mat();
        ImageUtil.gaussianBlur(gray, gsMat);

        // Sobel 运算，得到图像的一阶水平方向导数
        Mat sobel = new Mat();
        ImageUtil.sobel(gsMat, sobel);

        // 图像进行二值化
        Mat threshold = new Mat();
        ImageUtil.threshold(sobel, threshold);

        // 使用闭操作 同时处理一些干扰元素
        Mat morphology = threshold.clone();
        ImageUtil.morphologyClose(threshold, morphology); // 闭操作

        // 边缘腐蚀，边缘膨胀，可以多执行两次
        morphology = ImageUtil.erode(morphology,  4, 4);
        morphology = ImageUtil.dilate(morphology,4, 4, true);

        // 将二值图像，resize到原图的尺寸； 如果使用缩小后的图片提取图块，可能会出现变形，影响后续识别结果
        ImageUtil.enlarge(morphology, morphology, src.size());

        // 获取图中所有的轮廓
        List<MatOfPoint> contours = ImageUtil.contours(src, morphology);
        // 根据轮廓， 筛选出可能是车牌的图块
        Vector<Mat> blockMat = ImageUtil.screenBlock(src, contours, false);

        // 找出可能是车牌的图块，存到dst中， 返回结果
        hasPlate(blockMat, dst);
     ...
     * @return
     */
    public static Vector<Mat> findPlateByHsvFilter(String imagePath, Vector<Mat> dst, PlateHSV plateHSV) {
        Mat src = Imgcodecs.imread(imagePath);
        final Mat resized = ImageUtil.narrow(src, 600); // 调整大小,加快后续步骤的计算效率
        return findPlateByHsvFilter(src, resized, dst, plateHSV);
    }
     ...
     * @return
     */
    public static Vector<Mat> findPlateByHsvFilter(Mat src, Mat inMat, Vector<Mat> dst, PlateHSV plateHSV) {
        // hsv取值范围过滤
        Mat hsvMat = ImageUtil.hsvFilter(inMat, plateHSV.minH, plateHSV.maxH);
        // 图像均衡化
        Imgproc.cvtColor(hsvMat, hsvMat, Imgproc.COLOR_HSV2BGR);
        Mat equalizeMat = ImageUtil.equalizeHist(hsvMat);
        hsvMat.release();

        // 二次hsv过滤，二值化
        Mat threshold = ImageUtil.hsvThreshold(equalizeMat, plateHSV.equalizeMinH, plateHSV.equalizeMaxH);
        Mat morphology = threshold.clone();
        ImageUtil.morphologyClose(threshold, morphology); // 闭操作
        threshold.release();
        ...
        // 将二值图像，resize到原图的尺寸； 如果使用缩小后的图片提取图块，可能会出现变形，影响后续识别结果
        ImageUtil.enlarge(rgb, rgb, src.size());
        // 提取轮廓
        List<MatOfPoint> contours = ImageUtil.contours(src, rgb);
        // 根据轮廓， 筛选出可能是车牌的图块 // 切图的时候， 处理绿牌，需要往上方扩展一定比例像素
        Vector<Mat> blockMat = ImageUtil.screenBlock(src, contours, plateHSV.equals(PlateHSV.GREEN));
     ...
     *            包含车牌的图块
     */
    public static void hasPlate(Vector<Mat> inMat, Vector<Mat> dst) {
        for (Mat src : inMat) {
     ...
     * @return
     */
    public static PlateColor getPlateColor(Mat inMat, Boolean adaptive_minsv) {
        // 判断阈值
        final float thresh = 0.5f;
        // 转到HSV空间，对H均衡化之后的结果
        Mat hsvMat = ImageUtil.equalizeHist(inMat);

        if (colorMatch(hsvMat, PlateColor.GREEN, adaptive_minsv) > thresh) {
            return PlateColor.GREEN;
        }
        if (colorMatch(hsvMat, PlateColor.YELLOW, adaptive_minsv) > thresh) {
            return PlateColor.YELLOW;
        }
        if (colorMatch(hsvMat, PlateColor.BLUE, adaptive_minsv) > thresh) {
     ...
     * @return
     */
    public static Float colorMatch(Mat hsvMat, PlateColor r, Boolean adaptive_minsv) {
     ...
     * @param inMat 输入原始图像
     */
    public static String charsSegment(Mat inMat, PlateColor color) {
        ...
        ImageUtil.gaussianBlur(gray, gray);

        // 图像进行二值化 // 图像二值化阈值选取--未完成heima
        Mat threshold = new Mat();
        switch (color) {
        case BLUE:
        ...
        // 边缘腐蚀
        threshold = ImageUtil.erode(threshold,  2, 2);

        // 垂直方向投影，错切校正 // 理论上，还可以用于分割字符
        Integer px = getShearPx(threshold);
        ImageUtil.shearCorrection(threshold, threshold, px);

        // 前面已经结果错切校正了，可以按照垂直、水平方向投影进行精确定位
        // 垂直投影 + 垂直分割线，分割字符 // 水平投影，去掉上下边框、铆钉干扰
        threshold = sepAndClear(threshold, px, charCount);

        // 边缘膨胀 // 还原腐蚀操作产生的影响 // 会影响中文字符的精确度
        threshold = ImageUtil.dilate(threshold,  2, 2, true);
        ...
        // 识别字符，计算置信度
        List<PlateRecoResult> result = new ArrayList<>();
     ...
     */
    public static Mat sepAndClear(Mat threshold, Integer px, Integer charCount) {
     ...
     */
    public static Vector<Mat> getPlateMat(String imagePath, Vector<Mat> dst) {
        Mat src = Imgcodecs.imread(imagePath);
        final Mat resized = ImageUtil.narrow(src, 600); // 调整大小,加快后续步骤的计算效率

        CompletableFuture<Vector<Mat>> f1 = CompletableFuture.supplyAsync(() -> {
            Vector<Mat> r = findPlateByContours(src, resized, dst);
            return r;
        });
        CompletableFuture<Vector<Mat>> f2 = CompletableFuture.supplyAsync(() -> {
            Vector<Mat> r = findPlateByHsvFilter(src, resized, dst, PlateHSV.BLUE);
            return r;
        });
        CompletableFuture<Vector<Mat>> f3 = CompletableFuture.supplyAsync(() -> {
            Vector<Mat> r = findPlateByHsvFilter(src, resized, dst, PlateHSV.GREEN);
            return r;
        });
        CompletableFuture<Vector<Mat>> f4 = CompletableFuture.supplyAsync(() -> {
            Vector<Mat> r = findPlateByHsvFilter(src, resized, dst, PlateHSV.YELLOW);
            return r;
        });
